...
static const uint sock_filter_policy_fd_execle_tile_instr_cnt = 41;

static void populate_sock_filter_policy_fd_execle_tile( ulong out_cnt, struct sock_filter out[ static 41 ], uint logfile_fd, uint accounts_fd ) {
  FD_TEST( out_cnt >= 41 );
  struct sock_filter filter[41] = {
    ...
    /* allow mmap */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_mmap, /* RET_ALLOW */ 8, 0 ),
    /* allow madvise */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_madvise, /* RET_ALLOW */ 7, 0 ),
    /* check write */
