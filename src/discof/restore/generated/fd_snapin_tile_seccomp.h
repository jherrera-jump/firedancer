...
static const uint sock_filter_policy_fd_snapin_tile_instr_cnt = 31;

static void populate_sock_filter_policy_fd_snapin_tile( ulong out_cnt, struct sock_filter out[ static 31 ], uint logfile_fd, uint accounts_fd ) {
  FD_TEST( out_cnt >= 31 );
  struct sock_filter filter[31] = {
    ...
    /* allow mmap */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_mmap, /* RET_ALLOW */ 6, 0 ),
    /* allow madvise */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_madvise, /* RET_ALLOW */ 5, 0 ),
    /* check write */
