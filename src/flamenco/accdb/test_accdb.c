...
test_disjoint_index_rejects_unbacked_dirty_migration( void ) {
  ...
  fd_accdb_fork_id_t child = fd_accdb_attach_child( accdb, root );
  accdb_write( accdb, child, key, 20UL, NULL, 0UL, owner3 );

  uint dirty_idx = cold_map[ cold_hash ];
  FD_TEST( dirty_idx!=UINT_MAX && dirty_idx!=root_idx );
  FD_TEST( fd_accdb_acc_offset( &cold_pool[ dirty_idx ] )==FD_ACCDB_OFF_INVAL );
  FD_TEST( FD_ACCDB_SIZE_CACHE_VALID( cold_pool[ dirty_idx ].executable_size ) );

  /* Model the pre-fix eviction window after it severed the cache
     binding but before it published the replacement disk offset.  A
     migration must never publish this unbacked version in the other
     tier.  Production dirty eviction now retains CACHE_CLAIM across
     this entire window; the explicit state below is also rejected as a
     defensive invariant check. */
  cold_pool[ dirty_idx ].cache_idx = FD_ACCDB_ACC_CIDX_INVAL;
  cold_pool[ dirty_idx ].executable_size &= ~FD_ACCDB_SIZE_CACHE_VALID_BIT;
  ...
  FD_TEST( shmem->shmetrics->index_promotions==promotions_before );
  FD_TEST( shmem->shmetrics->index_blocked_migrations==blocked_before+1UL );
  FD_TEST( cold_map[ cold_hash ]==dirty_idx );
